...
@section('content')
    <main id="admin" class="base">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <h2 class="title">Nuevo Juego</h2>
                    <p><a href="/admin">Volver a Admin</a></p>
                    @if(session('msg'))
                        <p class="message">{{ session('msg') }}</p>
                    @endif
                    <form action="/guardar-juego" method="post">
                        @csrf
                        <label for="juego">Juego</label>
                        <input id="juego" type="text" name="nombre" maxlength="64" required>

                        <label for="imagen">Imagen</label>
                        <input name="imagen" type="text" id="imagen" maxlength="64">

                        <label for="reglas">Reglas (Separadas por | )</label>
                        <textarea class="w100" id="reglas" name="reglas"></textarea>

                        <label for="descripcion">Descripción</label>
                        <textarea class="w100" id="descripcion" name="descripcion"></textarea>
